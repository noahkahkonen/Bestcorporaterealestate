...
export default function EditListingPage() {
  ...
  useEffect(() => {
    Promise.all([
      ...
      try {
        ...
        if (Array.isArray(parsed)) feat = parsed;
      } catch {}
      let gall: string[] = [];
      try {
        const parsed = JSON.parse(l.galleryImagesJson || "[]");
        if (Array.isArray(parsed)) gall = parsed;
      } catch {}
      const brokerIds = (l.brokers || []).map((b: any) => b.agentId || b.agent?.id).filter(Boolean);
      setForm({
        title: l.title || "",
        address: l.address || "",
        city: l.city || "",
        state: l.state || "OH",
        zipCode: l.zipCode || "",
        latitude: String(l.latitude ?? "39.9612"),
        longitude: String(l.longitude ?? "-83.0007"),
        ...
        leasePricePerSf: l.leasePricePerSf != null ? String(Number(l.leasePricePerSf).toFixed(2)) : "",
  ...
  if (!listing || !form) {
    return (
      <div>
        <h1 className="text-2xl font-bold text-[var(--charcoal)]">Edit Listing</h1>
  ...
  return (
    <div>
      <div className="mb-6 flex items-center gap-4">
        ...
        <h1 className="text-2xl font-bold text-[var(--charcoal)]">Edit: {listing.title}</h1>
      </div>
      <form onSubmit={handleSubmit} className="space-y-6">
        <div className="rounded-xl border border-[var(--border)] bg-[var(--surface)] p-6">
          ...
          <div className="mt-4 grid gap-4 sm:grid-cols-2">
            ...
            <div>
              ...
              <input value={form.address} onChange={(e) => setForm((f) => ({ ...f!, address: e.target.value }))} required className="mt-1 w-full rounded-lg border px-3 py-2" />
            </div>
            <div>
        ...
        <div className="rounded-xl border border-[var(--border)] bg-[var(--surface)] p-6">
          ...
          <div className="mt-4 grid gap-4 sm:grid-cols-2">
            ...
            <div className="sm:col-span-2">
              <label className="block text-sm font-medium">Zoning</label>
              <input
                value={form.zoning}
        ...
        <div className="rounded-xl border border-[var(--border)] bg-[var(--surface)] p-6">
          ...
          <textarea value={form.description} onChange={(e) => setForm((f) => ({ ...f!, description: e.target.value }))} rows={4} className="mt-4 w-full rounded-lg border px-3 py-2" />
          <div className="mt-4">
        ...
        <div className="rounded-xl border border-[var(--border)] bg-[var(--surface)] p-6">
          ...
          <div className="mt-4">
                  ...
                  <div className="relative h-20 w-28 overflow-hidden rounded-lg border border-[var(--border)] bg-[var(--surface-muted)]">
                    ...
                    <button
                      ...
                      onClick={() =>
                        setForm((f) => ({
                          ...f!,
                          galleryImagesJson: f!.galleryImagesJson.filter((_, idx) => idx !== i),
                        }))
                      }
                      className="absolute right-1 top-1 flex h-6 w-6 items-center justify-center rounded-full bg-red-500 text-white opacity-90 hover:opacity-100"
            ...
            />
            {form.galleryImagesJson.length > 0 && (
              <p className="mt-1 text-xs text-[var(--charcoal-light)]">{form.galleryImagesJson.length} image(s)</p>
            )}
          </div>
          <div className="mt-4">
          ...
          <div className="mt-4">
            ...
            <input
              ...
              className="mt-1 w-full rounded-lg border px-3 py-2"
            />
            <p className="mt-1 text-xs text-[var(--charcoal-light)]">Optional. Paste a YouTube watch or embed URL to show a video on the listing page.</p>
          </div>
        </div>
