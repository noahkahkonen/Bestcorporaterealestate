...
export default function AdminListingsClient({
  ...
  async function handleMarkSold() {
    ...
    try {
      ...
      if (res.ok) {
        setListings((prev) => prev.map((l) => (l.id === soldModal.id ? { ...l, ...data } : l)));
        setSoldModal(null);
        setSoldForm({ soldPrice: "", soldDate: "", soldNotes: "", transactionOutcome: "Sold" });
      } else {
        alert((data as { error?: string }).error || "Failed to save. Please try again.");
      }
    } finally {
      setSavingSold(false);
    }
    ...
    <div>
      {loadError && (
        <div className="mb-6 rounded-xl border border-red-200 bg-red-50 p-4 dark:border-red-900/50 dark:bg-red-950/30">
          <p className="text-sm font-semibold text-red-800 dark:text-red-200">Could not load listings</p>
          <p className="mt-1 text-sm text-red-700 dark:text-red-300">{loadError}</p>
          <p className="mt-3 text-xs text-red-600 dark:text-red-400">
            If you recently deployed database changes (for example zoning fields), run{" "}
            <code className="rounded bg-red-100 px-1 py-0.5 dark:bg-red-900/50">npx prisma migrate deploy</code> on
            your production database, then refresh. The dashboard count can still work while listing queries fail if the
      ...
      <div className="mt-6 space-y-2">
        {!loadError && filteredListings.length === 0 ? (
        ...
        <div className="fixed inset-0 z-50 flex items-center justify-center bg-black/50">
          <div className="mx-4 max-w-md rounded-xl border border-[var(--border)] bg-[var(--surface)] p-6 shadow-xl">
            ...
            <div className="mt-6 flex gap-3">
              ...
              <button type="button" onClick={handleDelete} className="flex-1 rounded-lg bg-red-600 px-4 py-2.5 text-sm font-semibold text-white">
                Delete
              </button>
            </div>
          </div>
